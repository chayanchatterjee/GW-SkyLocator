...
class DataLoader:
    ...
    def load_train_data(data_config):
        ...
        # BBH
        f5 = h5py.File(data_config.BBH.path_train, 'r')
        ...
        h1_real_200k = abs(f5['h1_snr_series'][()])
        l1_real_200k = abs(f5['l1_snr_series'][()])
        v1_real_200k = abs(f5['v1_snr_series'][()])


        h1_real = np.concatenate([h1_real_52k, h1_real_30k, h1_real_12k, h1_real_6k, h1_real_200k], axis=0)
        l1_real = np.concatenate([l1_real_52k, l1_real_30k, l1_real_12k, l1_real_6k, l1_real_200k], axis=0)
        v1_real = np.concatenate([v1_real_52k, v1_real_30k, v1_real_12k, v1_real_6k, v1_real_200k], axis=0)
        ...
        h1_imag_200k = np.imag(f5['h1_snr_series'][()])
        l1_imag_200k = np.imag(f5['l1_snr_series'][()])
        v1_imag_200k = np.imag(f5['v1_snr_series'][()])
    
        h1_imag = np.concatenate([h1_imag_52k, h1_imag_30k, h1_imag_12k, h1_imag_6k, h1_imag_200k], axis=0)
        l1_imag = np.concatenate([l1_imag_52k, l1_imag_30k, l1_imag_12k, l1_imag_6k, l1_imag_200k], axis=0)
        v1_imag = np.concatenate([v1_imag_52k, v1_imag_30k, v1_imag_12k, v1_imag_6k, v1_imag_200k], axis=0)
    ...
    def load_train_parameters(data_config):
        ...
        ra_200k = 2.0*np.pi*f5['ra'][()]
        dec_200k = np.arcsin(1.0-2.0*f5['dec'][()])

        ra = np.concatenate([ra_52k, ra_30k, ra_12k, ra_6k, ra_200k])
        dec = np.concatenate([dec_52k, dec_30k, dec_12k, dec_6k, dec_200k])
    ...
    def load_3_det_samples(data_config, X_real, X_imag, y, num_samples, data):
        ...
        if(data == 'train'):
            ...
            h1_snr = np.concatenate([h1_snr_1, h1_snr_2, h1_snr_3, h1_snr_4, h1_snr_5])
            ...
            l1_snr = np.concatenate([l1_snr_1, l1_snr_2, l1_snr_3, l1_snr_4, l1_snr_5])
            ...
            v1_snr = np.concatenate([v1_snr_1, v1_snr_2, v1_snr_3, v1_snr_4, v1_snr_5])
            ...
            ra = np.concatenate([ra_1, ra_2, ra_3, ra_4, ra_5])
            dec = np.concatenate([dec_1, dec_2, dec_3, dec_4, dec_5])
