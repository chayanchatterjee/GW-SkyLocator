...
class WaveNet:
    ...
    def __init__(self, input1, input2, filters, kernel_size, activation):
    ...
    def construct_model(self):
        ...
        X = tf.keras.layers.Conv1D(filters = self.filters, kernel_size = self.kernel_size, padding='causal', 
                                       activation=self.activation, dilation_rate=2.0*self.dilation_rate)(X)
            
        X = tf.keras.layers.Conv1D(filters = self.filters, kernel_size = self.kernel_size, padding='causal', 
                                       activation=self.activation, dilation_rate=4.0*self.dilation_rate)(X)
        
        X = tf.keras.layers.Conv1D(filters = self.filters, kernel_size = self.kernel_size, padding='causal', 
                                       activation=self.activation, dilation_rate=8.0*self.dilation_rate)(X)
